...
// ------------------------------------------------------------
    ...
    ck_assert_int_eq(ckvs_local_get(NULL, 2, argv), ERR_INVALID_ARGUMENT);
    ck_assert_int_eq(ckvs_local_get(NON_EXISTING_CKVS, 1, argv), ERR_NOT_ENOUGH_ARGUMENTS);
    ck_assert_int_eq(ckvs_local_get(NON_EXISTING_CKVS, 3, argv), ERR_TOO_MANY_ARGUMENTS);
...
// ------------------------------------------------------------
    ...
    char* argv[] = { key, pwd, NULL };
    ck_assert_int_eq(ckvs_local_get(DUMMY_NAME, 2, argv), ERR_KEY_NOT_FOUND);
...
// ------------------------------------------------------------
    ...
    ck_assert_int_eq(ckvs_local_get(DUMMY_NAME, 2, argv), ERR_DUPLICATE_ID);
...
// ------------------------------------------------------------
    ...
    char* argv[] = { expected.key, pwd, NULL };
    ck_assert_int_eq(ckvs_local_get(DUMMY_NAME, 2, argv), ERR_INVALID_ARGUMENT);
...
// ------------------------------------------------------------
    ...
    char* argv[] = { expected.key, pwd, NULL };
    ck_assert_int_eq(ckvs_local_get(DUMMY_NAME, 2, argv), ERR_IO);
...
// ------------------------------------------------------------
    ...
    char* argv[] = { expected.key, pwd, NULL };
    ck_assert_int_eq(ckvs_local_get(DUMMY_NAME, 2, argv), ERR_NONE);
...
// ------------------------------------------------------------
    ...
    memcpy(ckvs.entries + 12, &expected, sizeof(ckvs_entry_t));
    ckvs.header.num_entries = 1;
...
#pragma GCC diagnostic pop
    ...
    tcase_add_test(tc1, get_present_key_2);
