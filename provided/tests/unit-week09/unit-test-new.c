...
// ------------------------------------------------------------
    ...
    char* argv[] = { key, pwd, NULL };
    ck_assert_int_eq(ckvs_local_new(NULL, 2, argv), ERR_INVALID_ARGUMENT);
    ck_assert_int_eq(ckvs_local_new(NON_EXISTING_CKVS, 0, argv), ERR_NOT_ENOUGH_ARGUMENTS);
    ck_assert_int_eq(ckvs_local_new(NON_EXISTING_CKVS, 1, argv), ERR_NOT_ENOUGH_ARGUMENTS);
    ck_assert_int_eq(ckvs_local_new(NON_EXISTING_CKVS, 3, argv), ERR_TOO_MANY_ARGUMENTS);
...
// ------------------------------------------------------------
    ...
    char* argv[] = { key, pwd, NULL };
    ck_assert_int_eq(ckvs_local_new(DUMMY_NAME, 2, argv), ERR_DUPLICATE_ID);
...
// ------------------------------------------------------------
    ...
    char* argv[] = { key, pwd, NULL };
    ck_assert_int_eq(ckvs_local_new(DUMMY_NAME, 2, argv), ERR_NONE);
...
// ======================================================================
START_TEST(new_non_existing_key_3)
{
// ------------------------------------------------------------
#ifdef WITH_PRINT
    printf("=== %s:\n", __func__);
#endif
    char k1[] = "adnr"; // hash = 2047
    char k2[] = "gaxu";
    char pwd[] = "00001111";

    init_ckvs(ckvs, 2048, 10); // size != 64
    strcpy(ckvs.entries[2047].key, k1);
    ckvs.header.num_entries = 1;
    create_file_and_dump_db(DUMMY_NAME, &ckvs.header, ckvs.entries);
    ...
    ckvs_sha_t auth_key = { {
        0xd5, 0x9c, 0xf8, 0x72, 0xad, 0x42, 0x3e, 0xdb, 0xdc, 0x4d, 0xa7, 0x69, 0x31, 0xf6, 0x09, 0x01, 
        0xc8, 0xdd, 0x83, 0xe2, 0xae, 0xee, 0xe3, 0x97, 0x7d, 0x21, 0x21, 0x50, 0x6c, 0x76, 0x39, 0x21
    } };
...
#pragma GCC diagnostic pop
    ...
    tcase_add_test(tc1, new_non_existing_key_3);
