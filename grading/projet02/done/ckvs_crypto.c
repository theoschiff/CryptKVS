...
#define CKVS_MAXCONCATLEN 2 * CKVS_MAXKEYLEN + 2
...
int ckvs_client_encrypt_pwd(ckvs_memrecord_t *mr, const char *key, const char *pwd)
{
    ...
    concatenation = calloc(2, CKVS_MAXKEYLEN + 1);
    if(concatenation == NULL){
        return ERR_OUT_OF_MEMORY;
    }
    strncat(concatenation, key, CKVS_MAXKEYLEN);
    strcat(concatenation, "|");
    strncat(concatenation, pwd, CKVS_MAXKEYLEN);
    ...
    SHA256((const unsigned char*) concatenation, strnlen(concatenation, CKVS_MAXCONCATLEN),
