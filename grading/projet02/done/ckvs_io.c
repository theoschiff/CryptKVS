...
int ckvs_find_entry(struct CKVS *ckvs, const char *key, const struct ckvs_sha *auth_key, struct ckvs_entry **e_out){
    ...
    for(int i = 0; i < (int) ckvs->header.table_size; ++i) {
...
int ckvs_write_encrypted_value(struct CKVS *ckvs, struct ckvs_entry *e, const unsigned char *buf, uint64_t buflen){
    ...
    if (error != 0){
        e->value_len = 0; //correcteur : plus facile de travailler sur une entry temporaire, puis copier à la fin si aucune erreur a eu lieu
        e->value_off = 0;
        ckvs_close(ckvs);
        return error;
    }
    ...
    if(error != ERR_NONE){
        ckvs_close(ckvs); // correcteur : du coup ici e->value/offset devrait etre 0
        return ERR_IO;
    }
    return ERR_NONE;
}
...
//correcteur : ne devrait pas incrementer num_entries en cas d'erreur
    ...
    strncpy(ckvsEntry.auth_key, auth_key);
...
//TODO finish coding this we want to write the header with the new size
static int ckvs_write_header_to_disk(const struct CKVS *ckvs){
    M_REQUIRE_NON_NULL(ckvs);
...
int power_of_two(int table_size){
    if((table_size & (table_size -1)) && (table_size != 1)){
        pps_printf("table size is not a power of 2"); //correcteur : devrait utiliser debug_print
        return 0;
    }
    return 1;
}
